...
_checkFrequencyAccelerated = 200;				// sleep accelerated
_playerThreshold = 50;							// How many players before accelerated cycle kicks in?

_deadMenLimit = -1;								// Bodies. Set -1 to disable.
_deadMenDistCheck = FALSE;						// TRUE to delete any bodies that are far from players.
...
_deadVehicleDistCheck = FALSE;					// TRUE to delete any destroyed vehicles that are far from players.
...
_craterDist = 1;								// Distance (meters) from players that craters are not deleted if below max.
