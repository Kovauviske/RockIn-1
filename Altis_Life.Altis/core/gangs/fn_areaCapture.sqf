...
if (_gangNum isEqualTo "Gang_Area_2") then {
    "gang_cap_2" setMarkerText _pText;
};

if (_gangNum isEqualTo "Gang_Area_3") then {
...
_xname = ""; //setup for popo name
_nearUnits = _area nearEntities ["Man",500]; //searches area for players
if (isNil {group player getVariable "gang_name"}) exitWith { hint "Você precisa estar numa gangue para capturar este cartel!"; }; //not in a gang
if (_group isEqualTo group player) exitWith { hint "Sua gangue já possui controle deste cartel!"; }; //already own it
if ((_area getVariable ["inCapture",FALSE])) exitWith {hint "Somente uma pessoa deve capturar por vez!"; }; //stops 2 people capturing at the same time
...
//check if area is contested
{
    ...
    if (_areaContested) exitWith {
        if (_xname isEqualTo "Police Force") then {
            hint "A Policia ta por perto! Livre-se deles primeiro.";
        } else {
            hint "Outros rebeldes estão por perto! Livre-se deles primeiro!";
        };
    };
